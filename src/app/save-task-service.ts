...
export class SaveTaskService {
  ...
  public loadTasks(project_id: string): void {
    this.http.get<any[]>(`https://todoback-2zof.onrender.com/getTasks/${project_id}`).subscribe(
      (res) => {
        console.log(res);
  ...
  addTask(task: any) {
    this.selectproject.subscribe((project) => {
      if (project) {
        ...
        this.http.post('https://todoback-2zof.onrender.com/addTask', task).subscribe(
          (res) => {
  ...
  update(task: any) {
    ...
    this.selectproject.subscribe((project) => {
      if (project) {
        console.log(task);
        this.http.put(`https://todoback-2zof.onrender.com/updateTask`, task).subscribe(
          (res) => {
            this.loadTasks(project.pid);
          },
          (error) => {
