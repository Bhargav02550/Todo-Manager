...
export class LeftSideComponent implements OnInit {
  ...
  refreshProjects() {
    this.http.get<ProjectType[]>('https://todoback-2zof.onrender.com/getProjects', {
  ...
  saveinput() {
    if (this.inputvalue.trim()) {
      ...
      this.http.post('https://todoback-2zof.onrender.com/addProject' , data_project).subscribe(
        (response) => {
          this.refreshProjects();  
          Toastify({
  ...
  delete(i: string): void {
    this.http.delete(`https://todoback-2zof.onrender.com/deleteProject/${i}`).subscribe(
      (res) => {
        this.refreshProjects();
        Toastify({
